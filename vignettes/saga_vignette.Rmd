...
## saga_gentargets
After pooling the samples in a specific folder (`samplepath`), a targets file called `SampleInformation.txt` must be provided for analysis. Without this file SAGA analysis will fail. Since its construction is a bit tedious, we provide a function for automatic generation of an empty file. However, the user will then have to adjust this file manually to his experimental setup since the function cannot know about the individual experimental designs. This includes the fields: Filename, Batch,	Group, Vector and TrueLabel.
...
saga_predict(samplepath, matrix.train, labels.train, matrix.unknown, pData.Test, writeFile=1, showRoc=0)
