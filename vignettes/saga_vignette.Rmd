...
Please note, that the SAGA package requires dependencies. Some functions can mask each other (depending on the local R setup). This may cause **warnings** (not errors). These can be ignored since they do not hamper the function of the SAGA package. Warnings may be caused mainly by the HTSanalyzeR dependency which has not yet been updated to work with R4.0.

**More importantly, the mnormt package has not yet been updated to R4.0 as well.** You can install a previous version of mnormt to run the saga package on R4.0 which can be installed with this code:
